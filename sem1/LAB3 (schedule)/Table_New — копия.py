...
# Вычисление пробелов для знач. по OY (ymin)
...
for i in range(1,Sequence):
    ...
    if A[i] < 0:
        CheckMin1 = 1
    if abs(A1[i]) > 99:
        spacesToNumbOY = (blocks + 7  + CheckMin1) - Seqi - qw
        A2[i] = '{:' + str(spacesToNumbOY) + '.1e}'
        Seqi = 7 + CheckMin1
    elif abs(A1[i]) > 10:
        A1[i] = int(A1[i])
        spacesToNumbOY = (blocks  + 1  + 1 + CheckMin1) - Seqi - qw
        A2[i] = '{:' + str(spacesToNumbOY) + 'd}'
        Seqi = 2  + CheckMin1
    else:
        spacesToNumbOY = (blocks   + 3 + CheckMin1)  - Seqi - qw
